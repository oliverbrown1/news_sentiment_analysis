...
SentimentLabel = ClassificationLabel
SentimentPrediction = ClassificationPrediction
...
def _classification_metrics(scored: list[_ScoredAnnotation]) -> dict[str, object]:
    common_scored = [
        ScoredPrediction(item.annotation.label, item.prediction) for item in scored
    ]
    return classification_metrics(common_scored, count_key="evaluated_annotations")
...
def _calibration_metrics(
    ...
    return calibration_metrics(common_scored, bins=bins)
...
def _latency_metrics(
    ...
    return latency_metrics(
        inference_times,
        model_load_seconds,
        count_key="annotations_succeeded",
        mean_key="mean_ms_per_annotation",
    )
